#!/usr/bin/env python3
"""Отдельный процесс MP4-рендера Remotion (не зависит от Flask/json-video)."""
...
def _reload_cancel_flag(st: dict[str, Any], job_id: str, task_id: str) -> dict[str, Any]:
    disk = load_render_state(job_id)
...
def run_render(job_id: str, task_id: str) -> int:
    global _RENDER_PROC
    props_path = job_remotion_dir(job_id) / "props.json"
    out_path = job_remotion_dir(job_id) / "out.mp4"
    log_path = render_log_path(job_id)

    st = load_render_state(job_id) or {}
    ...
    st["state"] = "running"
    st["stage"] = "starting"
    st["message"] = "Воркер рендера запущен"
    ...
    try:
        log_path.write_text(
            f"=== montage render worker pid={os.getpid()} job={job_id} task={task_id} ===\n",
            encoding="utf-8",
        )
    except OSError:
        pass
    ...
    with log_path.open("a", encoding="utf-8", buffering=1) as log_f:
        ...
        if _RENDER_PROC.stdout is not None:
            for raw in _RENDER_PROC.stdout:
                st = _reload_cancel_flag(st, job_id, task_id)
                if _apply_cancel(st):
    ...
    st = load_render_state(job_id) or st
    if _apply_cancel(st):
        st.update({
            "state": "cancelled",
            "stage": "cancelled",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Remotion montage render worker")
    ...
    if not job_id or not task_id:
        print("job-id and task-id required", file=sys.stderr)
        return 2

    def _on_term(_signum: int, _frame: Any) -> None:
        st = load_render_state(job_id)
    ...
    return run_render(job_id, task_id)
