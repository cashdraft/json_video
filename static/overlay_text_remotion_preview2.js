(function () {
    ...
    const root = document.querySelector('[data-overlay-text-agent="remotion-preview2"]');
    if (!root) return;

    const cm2Ta = document.querySelector('[data-cm2-result]');

    const els = {
        model: document.getElementById('rp2-model'),
        systemToggle: root.querySelector('[data-rp2-system-toggle]'),
        systemWrap: root.querySelector('[data-rp2-system-wrap]'),
        systemTa: root.querySelector('[data-rp2-system-prompt]'),
        systemBadge: root.querySelector('[data-rp2-system-badge]'),
        userToggle: root.querySelector('[data-rp2-user-toggle]'),
        userWrap: root.querySelector('[data-rp2-user-wrap]'),
        userTa: root.querySelector('[data-rp2-user-prompt]'),
        userBadge: root.querySelector('[data-rp2-user-badge]'),
        resultToggle: root.querySelector('[data-rp2-result-toggle]'),
        resultWrap: root.querySelector('[data-rp2-result-wrap]'),
        resultTa: root.querySelector('[data-rp2-result]'),
        resultBadge: root.querySelector('[data-rp2-result-badge]'),
        resultCounts: root.querySelector('[data-rp2-result-counts]'),
        resultCopy: root.querySelector('[data-rp2-result-copy]'),
        exportBtn: root.querySelector('[data-rp2-export]'),
        runBtn: root.querySelector('[data-rp2-run]'),
        statusRow: root.querySelector('[data-rp2-status-row]'),
        statusText: root.querySelector('[data-rp2-status-text]'),
        cancelBtn: root.querySelector('[data-rp2-cancel-btn]'),
        photoPreview: root.querySelector('[data-rp2-photo-preview]'),
        photoBadge: root.querySelector('[data-rp2-photo-badge]'),
    };
    ...
    function collectPayload(extra) {
        return Object.assign({
            rp2_model: els.model ? els.model.value : '',
            rp2_system_prompt: els.systemTa ? els.systemTa.value : '',
            rp2_user_prompt: els.userTa ? els.userTa.value : '',
            rp2_result: els.resultTa ? els.resultTa.value : '',
            cm2_result: getCm2Result(),
        }, extra || {});
    }

    async function savePrefs(extra) {
        const r = await fetch('/overlay-text/api/remotion-preview2/prefs', {
            method: 'POST',
    ...
    function bindLockToggle(toggle, wrap, ta, onSave, opts) {
        ...
        setLocked(toggle.classList.contains('rewrite-lock-toggle--locked'));
    ...
    async function downloadExport(btn) {
        ...
        try {
            await savePrefs();
            const r = await fetch('/overlay-text/api/remotion-preview2/export', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            a.href = url;
            a.download = 'overlay_text_remotion_preview2_request.json';
    ...
    async function runAgent() {
        ...
        try {
            const r = await fetch('/overlay-text/api/remotion-preview2/generate', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(collectPayload()),
            ...
            if (els.resultTa) els.resultTa.value = data.result || data.rp2_result || '';
    ...
    bindModeDropdown(root.querySelector('[data-rp2-model-field]'));
    ...
    [els.systemTa, els.userTa].forEach(function (el) {
        if (!el) return;
